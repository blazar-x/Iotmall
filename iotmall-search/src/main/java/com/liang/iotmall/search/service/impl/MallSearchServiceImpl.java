...
import com.liang.iotmall.search.feign.ProductFeignService;
...
import com.liang.iotmall.search.vo.AttrResponseVo;
...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        //2.执行检索请求
        try {
            SearchResponse response = client.search(searchRequest, IotmallElasticSearchConfig.COMMON_OPTIONS);
     ...
     * 准备检索请求,此代码为copy
     * param is null
     * @return: com.liang.iotmall.search.vo.SearchResult
     */
    ...
    private SearchResult buildSearchReSult( SearchResponse response,SearchParam param) {
        ...
        if (param.getAttrs() != null && param.getAttrs().size() > 0) {
            List<SearchResult.NavVo> collect = param.getAttrs().stream().map(attr -> {
                ...
                navVo.setLink("http://search.iotmall.com/list.html?" + replace);
